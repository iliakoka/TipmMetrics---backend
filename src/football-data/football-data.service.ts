...
export class FootballDataService {
  ...
  /**
   * Dynamically resolve a team logo URL by checking local DB fixtures first,
   * then falling back to API-Football team search if not found.
   */
  async searchTeamLogo(teamName: string): Promise<string | null> {
    if (!teamName) return null;
    const cleanName = teamName.trim().toLowerCase();
    if (this.logoCache.has(cleanName)) {
      return this.logoCache.get(cleanName) || null;
    }

    try {
      // 1. Check if this team exists in our PostgreSQL fixtures table
      const fixtureMatch = await this.fixtureRepository
        .createQueryBuilder('f')
        .where(
          '(LOWER(f.homeTeamName) = :name AND f.homeTeamLogo IS NOT NULL) OR (LOWER(f.awayTeamName) = :name AND f.awayTeamLogo IS NOT NULL)',
          { name: cleanName },
        )
        .getOne();
      ...
      if (fixtureMatch) {
        const logo =
          fixtureMatch.homeTeamName?.toLowerCase() === cleanName
            ? fixtureMatch.homeTeamLogo
            : fixtureMatch.awayTeamLogo;
        if (logo) {
          this.logoCache.set(cleanName, logo);
          return logo;
        }
      }

      // 2. Query API-Football for the team name
      const data = await this.rateLimitedGet('/teams', { search: teamName.trim() });
      if (Array.isArray(data) && data.length > 0) {
        // Match by exact name first, otherwise use first search result
        const exact = data.find((item: any) => item.team?.name?.toLowerCase() === cleanName);
        const logo = exact?.team?.logo || data[0]?.team?.logo || null;
        if (logo) {
          this.logoCache.set(cleanName, logo);
          return logo;
        }
      ...
      this.logger.warn(`Error resolving logo for team "${teamName}": ${err.message}`);
    }
